...
class RigpixCog(commands.Cog):
    ...
    async def _rigpix_slash(
        self, ctx: ApplicationContext, brand: str = "", model: str = ""
    ):
    ...
    async def _rigpix_prefix(
        self, ctx: commands.Context, brand: str = "", model: str = ""
    ):
        """Looks up equipment by make & model on RigPix and returns its specifications."""
        embed = await self._rigpix_core(ctx, brand, model)
        await ctx.send(embed=embed)

    async def _rigpix_core(
        self,
        ...
        if model_obj:
            ...
            embed.description = f"Could not find a RigPix entry for {brand} {model}."
            return embed
        ...
        async with self.session.get(model_link) as resp:
            ...
            for row in rows:
                ...
                table_dict[columns[0].text if 0 < len(columns) else ""] = (
                    columns[1].text if 1 < len(columns) else ""
                )
